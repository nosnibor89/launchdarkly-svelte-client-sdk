...
import { LD } from '../launchDarkly.js';
	...
	describe('createLD', () => {
		...
		describe('initialize', async () => {
			...
			it('should throw an error if the client is not initialized', async () => {
				ld = LD;
				expect(() => ld.watch('test-flag')).toThrow('LaunchDarkly client not initialized');
			...
			it('should set the loading status to false when the client is ready', async () => {
				const initializing = ld.initializing;
			...
			it('should call waitUntilReady when initializing', async () => {
			...
			it('should register an event listener for the "change" event', async () => {
